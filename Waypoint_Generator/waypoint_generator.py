...
FILE_PATH = (
 "~/workspace/metronome_ws/src/metronome_controller/waypoints_ex.csv"
)
...
def check_intercept(waypoints1, waypoints2):
 ...
 crash = (x1[i + 1], y1[i + 1]) == (x2[i], y2[i]) or (x1[i], y1[i]) == (
 x2[i],
 y2[i],
 ) # or (x1[i], y1[i]) == (x2[i+1], y2[i+1])

 if crash:
 if (x1[i + 1], y1[i + 1]) == (x2[i], y2[i]):
 print(
 f"Robots hit eachother between step {i}-{i+1} around {(x1[i], y1[i])}-{(x1[i+1], y1[i+1])}"
 )
 else:
...
def plot_robot_trajectory(waypoints, robot_name, ax=None):
 ...
 linestyle="-",
 ) # Plot the trajectory using lines
 ax.scatter(
 waypoints[:, 0],
 waypoints[:, 1],
 c=vector_color,
 label=f"{robot_name} Waypoints",
 marker="o",
 ) # Plot individual dots at each waypoint
 ax.scatter(
 waypoints[0, 0],
 waypoints[0, 1],
 c="green",
...
def define_waypoint_order_grid(x_points, y_points, option):
 ...
 coordinates_final = optimize_waypoint_order_grid(
 increment, x_points_new, y_points_new
 )
...
def optimize_waypoint_order_grid(increment, x_points, y_points):
 ...
 length = (
 coordinates.shape[0] if increment == "lr_movement" else coordinates.shape[1]
 )
 final_ordered_coords = []
 for i in range(length):
 line = (
 coordinates[i, :, :] if increment == "lr_movement" else coordinates[:, i, :]
 ) # row of our np array
 if i % 2 != 0:
 line = line[::-1]
 final_ordered_coords.append(line)
 return np.concatenate(
 final_ordered_coords, axis=0
 ) # after rearranging order of coord shape into sequential order ie [(x1,y1), (x2,y2)]
...
def generate_exhaustive_waypoints(params, bounds):
 ...
 x_points, y_points = np.linspace(0, x_spec["length"], x_spec["step"]), np.linspace(
 0, y_spec["length"], y_spec["step"]
 )
...
def combine_trans_rot_waypoint(coordinates, yaw_spec, el_spec, az_spec):
 ...
 coord_yaw_az_el_permutation = list(
 product(coordinates, yaw_points, az_points, el_points)
 )
 waypoints = np.array(
 [np.hstack(coord_yaw_az_el) for coord_yaw_az_el in coord_yaw_az_el_permutation]
 ) # make it a 2d array
 return waypoints
...
def generate_params_bounds(option1, option2):
 ...
 bounds1 = (
 {"min": 0, "max": 0, "step": 1},
 {"min": -150, "max": 150, "step": 6},
 {"min": -80, "max": 80, "step": 8},
 )
 ...
 sin_start1, num_points1, num_waves1, wave_dist1, direction1 = (
 (5, 6),
 900,
 10,
 5,
 ...
 bounds1 = (
 {"min": -180, "max": 180, "step": 1},
 {"min": -150, "max": 150, "step": 8},
 {"min": -80, "max": 80, "step": 6},
 )
 ...
 {"min": -80, "max": 80, "step": 20},
 )
 elif option2 == "Sinusoidal":
 sin_start2, num_points2, num_waves2, wave_dist2, direction2 = (
 (-5, 11),
 600,
 3,
 2,
...
def generate_waypoints():
 ...
 option1, option2 = path_options[0], path_options[0]
 ...
 bounds1,
 bounds2,
 ) = generate_params_bounds(option1, option2)
 ...
 r1_waypoints, r2_waypoints = custom_paths(
 (option1, option2), (bounds1, bounds2), (option1_params, option2_params)
 )
