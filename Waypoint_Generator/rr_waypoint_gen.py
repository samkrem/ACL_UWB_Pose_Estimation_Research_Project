...
DATA_TITLES=["t","x1", "y1","heading1","az1", "el1","x2", "y2", "heading2","az2", "el2",  "dist"]
def save_waypoints_as_csv(waypoints1, waypoints2):
    ...
    df = pd.DataFrame({
        ...
        DATA_TITLES[1]: x1,
        DATA_TITLES[2]: y1,
        DATA_TITLES[3]: heading1,
        DATA_TITLES[4]: az1,
        DATA_TITLES[5]: el1,
        DATA_TITLES[6]: x2,
        DATA_TITLES[7]: y2,
        DATA_TITLES[8]: heading2,
        DATA_TITLES[9]: az2,
        DATA_TITLES[10]: el2,
        DATA_TITLES[11]: dist
...
def generate_exhaustive_waypoints(params, bounds):
    ...
    waypoints = combine_trans_rot_waypoint(coordinates, az_spec, el_spec) #yaw_spec
    return waypoints
def generate_stationary_waypoints(params,bounds):
    ...
    waypoints = combine_trans_rot_waypoint(coordinates, el_spec, az_spec) #, yaw_spec
    return waypoints
...
def generate_circular_waypoints(params, bounds):
    ...
    waypoints = combine_trans_rot_waypoint(coordinates, el_spec, az_spec)
    return waypoints
def generate_sinusoidal_waypoints(params, bounds = None):
    ...
    coordinates = start + np.array(coordinates)
    waypoints = combine_trans_rot_waypoint(coordinates, az_spec, el_spec)
    return waypoints
def create_headings(coordinates):
    ...
    return headings
def combine_trans_rot_waypoint(coordinates, az_spec, el_spec):
    """
    ...
    headings = create_headings(np.array(coordinates))
...
def generate_params_bounds(option1, option2):
    ...
    if option2 == "Stationary":
        ...
        sin_start2, num_points2, num_waves2, wave_dist2, direction2 =(-5,11), 100, 3, 6, "vertical"
        params2 = (sin_start2, num_points2 ,num_waves2, wave_dist2, direction2)
        bounds2 = {'length': 8}, {'length': 8}, {'min': -150, 'max': 150, 'step': 3}, {'min': -80, 'max': 80, 'step': 2}  #{'min': -180, 'max': 180, 'step': 1}, 
...
def generate_waypoints():
    ...
    option1, option2 = path_options[0], path_options[2]
