...
class MultiAgentOptimization():
    ...
    #     x_guess = np.linspace(self.x0, self.xf, self.N)
    ...
    def solve_opt(self):
        ...
        # np.linspace(self.x0, self.xf, self.N )
        sol = self.opti.solve()
    ...
    def setup_mpc_opt(self, x0, xf, tf, waypoints={}, Qf=None, Q_waypoints={}, R=None, x_bounds=None, u_bounds=None):
        ...
        for m in range(self.M):
            x_guess = np.linspace(x0, xf, self.N).T  # x0 to xf over N+1 timesteps
        
            # Set initial guess for state (x)
            self.opti.set_initial(self.x[m], x_guess)
    ...
    def setup_min_time_opt(self, x0, xf, tf_guess=10.0, x_bounds=None, u_bounds=None):
        ...
        # Create a 2D array for the full initial guess (51, 4) for all agents.
        initial_guess = np.zeros((self.N+1, self.dynamics.x_shape))

        for i in range(self.dynamics.x_shape):
            # Stack the interpolated states for each state dimension
            x0_i = x0[:, i][0]  # shape (51,)
            xf_i = xf[:, i][0]  # shape (51,)

            initial_guess[:, i] = np.linspace(x0_i, xf_i, self.N+1)

        # Now set this initial guess to all agents
        for m in range(self.M):
            for i in range(self.dynamics.x_shape):
                # Set the initial guess for each agent and state dimension
                self.opti.set_initial(self.x[m][i,:], initial_guess[:, i])        
